...
Broadcaster::Broadcaster(QJsonObject const& a_config) {
	spdlog::trace("Broadcaster::Broadcaster()");
	configure(a_config);
}
...
void Broadcaster::configure(QJsonObject const& a_config) {
	...
	onConnect();
	spdlog::trace("Broadcaster::configure() ip:{} port:{}", m_ip.toStdString(),
...
void Broadcaster::onSubscribe(qint32 topic) {
	spdlog::debug("Broadcaster::onSubscribe() topic:{}", topic);
...
void Broadcaster::onUnsubscribe(qint32 topic) {
	spdlog::debug("Broadcaster::onUnsubscribe() topic:{}", topic);
	QVector<qint32> topics{};
	topics.push_back(topic);
...
void Broadcaster::onNewMessage(QByteArray const a_message) {
	...
	if (ret <= 0) {
		spdlog::warn("Broadcaster::onNewMessage() msg not correct");
	}
	else {
		...
		if (message.isValid()) {
			...
			if (!jDoc.isObject()) {
				spdlog::trace("Broadcaster::onNewMessage() Recived invalid Message");
			}
	...
	spdlog::trace("Broadcaster::onNewMessage from:{} \n{}", message.topic(),
...
void Broadcaster::onSendCommand(const qint32 topic, const QJsonObject json) {
	...
	spdlog::trace("Broadcaster::onSendCommand to:{} \n{}", topic,
		stateData.toStdString().c_str());
