...
void Broadcaster::configure(QJsonObject const &a_config) {
  H_Logger->trace("Broadcaster::configure()");
  ...
  onConnect();
  H_Logger->trace("Broadcaster::configure() ip:{} port:{}", m_ip.toStdString(),
...
void Broadcaster::onSubscribe(qint32 topic) {
  H_Logger->debug("Broadcaster::onSubscribe() topic:{}", topic);
...
void Broadcaster::onUnsubscribe(qint32 topic) {
  H_Logger->debug("Broadcaster::onUnsubscribe() topic:{}", topic);
  QVector<qint32> topics{};
  topics.push_back(topic);
...
void Broadcaster::onNewMessage(QByteArray const a_message) {
  ...
  if (ret <= 0) {
    H_Logger->warn("Broadcaster::onNewMessage() msg not correct");
  } else {
    ...
    if (message.isValid()) {
      ...
      if (!jDoc.isObject()) {
        H_Logger->trace("Broadcaster::onNewMessage() Recived invalid Message");
      }
  ...
  H_Logger->trace("Broadcaster::onNewMessage from:{} \n{}", message.topic(),
...
void Broadcaster::onSendCommand(const qint32 topic, const QJsonObject json) {
  ...
  H_Logger->trace("Broadcaster::onSendCommand to:{} \n{}", topic,
                  stateData.toStdString().c_str());
